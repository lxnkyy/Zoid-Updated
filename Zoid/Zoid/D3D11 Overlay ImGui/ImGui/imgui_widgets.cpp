...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImColor(15, 70, 255) : ImColor(15, 70, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? ImColor(15, 70, 255, 0) : ImColor(15, 70, 255, 0), g.IO.DeltaTime * 6.f);


    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.background), true, 6.f);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), ImColor(255, 255, 255), label);
...
bool ImGui::ButtonInfo(const char* label, const char* label_1, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImColor(20, 21, 22) : ImColor(20, 21, 22), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? ImColor(133, 133, 133, 255) : ImColor(255, 255, 255, 255), g.IO.DeltaTime * 6.f);


    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.background), true, 4.f);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 20, bb.Min.y + 10), ImColor(255, 255, 255), label);
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 20, bb.Min.y + 30), ImColor(255, 255, 255), label_1);
...
struct TachBar_state{

    ImVec4 background, text;

};
bool ImGui::TachBar(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::map<ImGuiID, TachBar_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, TachBar_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? ImColor(15, 70, 255, 255) : ImColor(64, 131, 228, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? ImColor(133, 133, 133, 255) : ImColor(255, 255, 255, 255), g.IO.DeltaTime * 6.f);
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.background), true, 3.f);

    ImGui::GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(0, 0), bb.Max + ImVec2(0, 0), ImVec2(0, 0), ImVec2(1, 1), ImColor(205,205,205,255));
...
bool ImGui::TachRoll(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? ImColor(64, 131, 228, 255) : ImColor(64, 131, 228, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? ImColor(133, 133, 133, 255) : ImColor(255, 255, 255, 255), g.IO.DeltaTime * 6.f);
...
bool ImGui::Tab(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg)
{
    ...
    const ImRect image_bb(pos, pos + ImVec2(30, 50));
    ...
    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? c::other::Text_Blue : c::other::Icon_In, g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(0, 0), bb.Min + ImVec2(30, 30), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));

    //GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 41, bb.Max.y - CalcTextSize(label).y - (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);
...
bool ImGui::SubTab(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::other::Text_Blue : hovered ? c::other::Text_Blue : c::other::text_ins, g.IO.DeltaTime * 6.f);
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? c::other::Text_Blue : c::other::ico_ins, g.IO.DeltaTime * 6.f);
    it_anim->second.anim = ImLerp(it_anim->second.anim, selected ? 15.f : 0.f, g.IO.DeltaTime * 6.f);


    window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 5, bb.Min.y + 26), ImVec2(bb.Min.x + 9 + it_anim->second.anim, bb.Max.y + 5), GetColorU32(c::other::Text_Blue));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() - 186, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 4.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? ImColor(255, 255, 255, 255) : ImColor(255, 255, 255, 76), g.IO.DeltaTime * 10.f);
    ...
    if (!is_multiline)
    {
        ...
        //window->DrawList->AddRect(ImVec2(frame_bb.Min.x - 2, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), ImColor(15, 70, 255, 255), 3.f);
        window->DrawList->AddShadowRect(ImVec2(frame_bb.Min.x - 2, frame_bb.Max.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), ImColor(15, 70, 255),30.f,ImVec2(0,0));
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x - 2, frame_bb.Max.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), ImColor(15, 70, 255));
        RenderFrame(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(20, 20, 20, 0), false, 3.f);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(4, 1), rect.Max + ImVec2(14, 13), ImColor(10, 10, 10, 100));
                }
        ...
        {
            it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? ImColor(255, 255, 255) : hovered ? ImColor(255, 255, 255) : ImColor(120, 120, 120), g.IO.DeltaTime * 6.f);

            ImU32 col = GetColorU32(it_anim->second.text_color);

            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(0, 2), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 1, 8), frame_bb.Min + ImVec2(it_anim->second.value + 2, 20), ImColor(255, 255, 255));
            }
    ...
    //window->DrawList->AddText(ImVec2(frame_bb.Max.x - CalcItemWidth() + 186, frame_bb.Min.y - 25), GetColorU32(c::text_white), label);
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, const ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
    ...
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
    {
        ...
        //GetForegroundDrawList()->AddRect(ImVec2(x1, tab_bar->BarRect.Min.y), ImVec2(x2, tab_bar->BarRect.Max.y), IM_COL32(15, 70, 255, 255));
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    if (out_text_clipped)
    {
        *out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_pixel_clip_bb.Max.x;
        //draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(15, 70, 255, 255) : IM_COL32(0, 255, 0, 255));
    }
